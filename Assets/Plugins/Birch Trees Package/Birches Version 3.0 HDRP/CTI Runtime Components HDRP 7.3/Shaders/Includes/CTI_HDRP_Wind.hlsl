...
//			float4 _TimeParameters; // already defined.
...
float3 origPos = position;
	...
	// 	Motionvectors rely on _LastTimeParameters.xyz
		if(timeParams.x != _Time.y) {
	//		timeParams.xyz = _LastTimeParameters.xyz
		}
	#endif
...
//  unity_ObjectToWorld does not work... but at least we can use the functions from HDRP!
	...
	float2 vWavesIn = timeParams.xx * float2(10,10) + float2(fVtxPhase, fBranchPhase);

	//#if (SHADERPASS == SHADERPASS_MOTION_VECTORS)
	//vWavesIn = _LastTimeParameters.xx + float2(fVtxPhase, fBranchPhase);
	//#else
	//vWavesIn = _TimeParameters.xx + float2(fVtxPhase, fBranchPhase);
	//#endif
	vWavesIn = timeParams.xx + float2(fVtxPhase, fBranchPhase);
...
//	FastSign may corrupt motion vectors?!
	float3 bend = Flutter * fDetailAmp * normal * sign(normal) * windMultipliers.z;
