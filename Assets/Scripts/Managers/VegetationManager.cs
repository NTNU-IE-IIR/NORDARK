...
public class VegetationManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        biomes = new Dictionary<string, AwesomeTechnologies.VegetationSystem.BiomeType>() {
            ...
            {"Forest", AwesomeTechnologies.VegetationSystem.BiomeType.BorealForest},
    ...
    {
        if (biomeArea.Name == "") {
            biomeArea.Name = DetermineNewBiomeAreaName();
        }
    ...
    public void AddNodeToCurrentBiomeArea(Vector3 position)
    {
        int biomeAreaIndex = vegetationControl.GetBiomeAreaIndex();
        if (biomeAreaIndex > -1 && biomeAreaIndex < biomeAreas.Count) {
            Vector2d coordinate = mapManager.GetCoordinatesFromUnityPosition(position);
    ...
    private string DetermineNewBiomeAreaName(int index = 1)
    {
        foreach (BiomeArea biomeArea in biomeAreas) {
            if (biomeArea.Name == "Biome " + index.ToString()) {
                return DetermineNewBiomeAreaName(index + 1);
            }
        }
        return "Biome " + index.ToString();
    }
}
